...
- **Security headers middleware** (`Program.cs`, before `UseAuthentication`): sets `X-Content-Type-Options`, `X-Frame-Options: DENY`, `Referrer-Policy`, and a `Content-Security-Policy` on every response. The CSP allow-lists exactly three third parties — `cdn.jsdelivr.net` (Bootstrap Icons), `challenges.cloudflare.com` (Turnstile script + iframe), and `calendar.google.com` (the `/calendar` iframe embed) — and allows `'unsafe-inline'` for `style-src` only (not `script-src`) to accommodate existing inline `style="..."` attributes. Adding a new external script/stylesheet/iframe source anywhere in the app requires updating this policy or it will be silently blocked by the browser.
